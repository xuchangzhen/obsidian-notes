...
另外 我们还有一个常用的参数**TC标志**（Transmission Complete）：代表整个数据帧发送完毕 
发送数据时 我们用**TXE**确保寄存器里面的数据不被替换 **TC**确保寄存器里的数据帧发送完毕
...
- 表示接收到数据寄存器中有新数据  
- **接收方式**：  
1. **轮询方式**：循环检测 RXNE，收到就读取数据  
2. **中断方式**：RXNE 置位时触发中断，自动读取数据  
3. **DMA方式**：接收数据自动写入缓冲区，CPU 不用一直轮询
...
| 特性   | 异步通信（UART 异步） | 同步通信（USART 同步/I²C/SPI）    |
| ---- | ------------- | ------------------------- |
